...
- **k8s 三件套按 patch 精确锁定**：非 `--only-addons` 构建且指定 `--kubernetes-version` 时，kubeadm/kubelet/kubectl 严格锁定到对应 patch 的包版本（apt：`name=X.Y.Z-1.1`，dnf/yum：`name-X.Y.Z-1.1`，如 `kubeadm=1.31.6-1.1` / `kubeadm-1.31.6-1.1`），避免同 minor 最新 patch 混入；源内无该 patch 时 apt/dnf 依赖解析失败即构建失败，不会静默回退同 minor 最新 patch。
